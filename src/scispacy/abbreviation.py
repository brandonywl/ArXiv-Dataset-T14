'''
Nmslib is a dependency of scispacy. There are documented issues of the installation with non-trivial fixes.
https://github.com/nmslib/nmslib/issues/464
Due to issues installing nmslib on Windows python and the non-support of nmslib for python > 3.9, we have opted to place
the code from scispacy repository into this folder instead. We are only using the abbreviation detector pipe from it and
are citing it here.

https://github.com/allenai/scispacy/blob/main/scispacy/abbreviation.py

None of this code is written by us.
'''
